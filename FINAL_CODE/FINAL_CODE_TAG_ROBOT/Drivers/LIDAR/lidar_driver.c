...
void store_points_data(h_lidar_t *lidar, points_sample_t *points,uint8_t point_nb)
{
	...
	{//Circle looped back -> calculate the moving average of the 2d map and store current 2D map
		xSemaphoreGive(find_target_semaphore);//Authorize the target finding algorithm
	}
}
